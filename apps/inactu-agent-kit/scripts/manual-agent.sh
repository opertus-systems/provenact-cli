...
DEFAULT_REQUEST="$ROOT_DIR/apps/inactu-agent-kit/examples/manual-agent-request.json"
...
if [[ "$TOOL_NAME" != "inactu.execute_skill" ]]; then
...
"$ROOT_DIR/apps/inactu-agent-kit/scripts/run-direct.sh" "$OUT_DIR" "$RECEIPT_PATH"
