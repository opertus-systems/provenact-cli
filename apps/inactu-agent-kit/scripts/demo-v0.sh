...
FIXTURE_DIR="$ROOT_DIR/apps/inactu-agent-kit/fixtures/echo-v0"
CLI=(cargo run -q -p inactu-cli --)
...
"$ROOT_DIR/apps/inactu-agent-kit/scripts/manual-agent.sh" \
  "$ROOT_DIR/apps/inactu-agent-kit/examples/manual-agent-request.json" \
  "$MANUAL_DIR" \
  "$MANUAL_RECEIPT"

"$ROOT_DIR/apps/inactu-agent-kit/scripts/run-direct.sh" \
  "$DIRECT_DIR" \
  "$DIRECT_RECEIPT"

"$ROOT_DIR/apps/inactu-agent-kit/scripts/mcp-tool-adapter.sh" \
  "$ROOT_DIR/apps/inactu-agent-kit/examples/mcp-tools-call.json" \
  "$MCP_DIR" \
  "$MCP_RECEIPT"
