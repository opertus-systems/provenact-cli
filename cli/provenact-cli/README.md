...
Minimal CLI for Provenact v0 verification workflows.
...
- stores installed content under `~/.provenact/store/sha256/<hash>/`
  (or `$PROVENACT_HOME/store/sha256/<hash>/`)
- updates local metadata index at `~/.provenact/index.json`

`export agentskills` bridges installed skills into filesystem-scanned AgentSkills
layouts while keeping execution in Provenact wrappers:
- source of truth: installed skills from `~/.provenact/index.json`
...
- wrappers call `provenact-cli run` with an exported `ide-safe` policy profile
...
1. `provenact-cli pack --bundle ./bundle --wasm ./skill.wasm --manifest ./manifest.json`
2. `provenact-cli sign --bundle ./bundle --signer alice.dev --secret-key ./alice.key`
3. `provenact-cli archive --bundle ./bundle --output ./skill.tar.zst`
4. `KEYS_DIGEST=\"$(shasum -a 256 ./public-keys.json | awk '{print \"sha256:\"$1}')\"`
5. `provenact-cli install --artifact ./skill.tar.zst --keys ./public-keys.json --keys-digest \"$KEYS_DIGEST\" --require-signatures`
