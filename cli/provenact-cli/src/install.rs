...
pub fn install(request: InstallRequest<'_>) -> Result<String, String> {
    ...
    let provenact_home = resolve_provenact_home()?;
...
fn resolve_provenact_home() -> Result<PathBuf, String> {
    if let Some(path) = std::env::var_os("PROVENACT_HOME") {
        return Ok(PathBuf::from(path));
    }
    let Some(home) = std::env::var_os("HOME") else {
        return Err("could not resolve home directory; set PROVENACT_HOME".to_string());
    };
    Ok(PathBuf::from(home).join(".provenact"))
}
...
fn persist_to_store(
    ...
    let store_dir = provenact_home.join("store").join("sha256").join(suffix);
...
fn update_index(
    ...
    fs::create_dir_all(provenact_home).map_err(|e| {
        format!(
            "failed to create provenact home {}: {e}",
            provenact_home.display()
        )
    })?;
