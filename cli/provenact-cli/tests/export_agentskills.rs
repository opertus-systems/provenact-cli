...
fn make_index_and_store(base: &Path) -> (std::path::PathBuf, String, std::path::PathBuf) {
    let provenact_home = base.join("provenact-home");
...
fn export_agentskills_codex_repo_writes_expected_layout() {
    ...
    let (provenact_home, digest, store) = make_index_and_store(&root);
    ...
    assert!(run_sh.contains("provenact-cli"), "{run_sh}");
    assert!(
    ...
    assert!(policy.contains("/tmp/provenact-scratch"), "{policy}");
...
fn export_agentskills_claude_user_targets_home_dot_claude() {
    ...
    let (provenact_home, _digest, _store) = make_index_and_store(&root);
