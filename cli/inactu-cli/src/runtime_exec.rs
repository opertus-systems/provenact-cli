...
use inactu_verifier::{sha256_prefixed, Capability};
use url::Url;
use wasmtime::{
...
fn kv_root() -> PathBuf {
    env::var("INACTU_KV_DIR")
        .map(PathBuf::from)
        .unwrap_or_else(|_| PathBuf::from("/tmp/inactu-kv"))
}
...
fn queue_root() -> PathBuf {
    env::var("INACTU_QUEUE_DIR")
        .map(PathBuf::from)
        .unwrap_or_else(|_| PathBuf::from("/tmp/inactu-queue"))
}
