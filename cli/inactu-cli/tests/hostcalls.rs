...
use inactu_verifier::{parse_receipt_json, sha256_prefixed};
use serde_json::json;
...
fn hostcalls_fs_roundtrip_receipts() {
  ...
  (data (i32.const 0) "/tmp/inactu-fs/itest.txt")
  ...
  (func (export "run") (result i32)
    ...
    let write_bundle_ctx = create_bundle(
        ...
        "fs-write",
        fs_write_wat,
        json!([{"kind":"fs.write","value":"/tmp/inactu-fs"}]),
    );
    let read_bundle_ctx = create_bundle(
        &root,
        "fs-read",
        fs_read_wat,
        json!([{"kind":"fs.read","value":"/tmp/inactu-fs"}]),
    );
    ...
    let policy = json!({
      ...
      "capability_ceiling": {
        "fs": {"read": ["/tmp/inactu-fs"], "write": ["/tmp/inactu-fs"]}
      }
    });
    ...
    write(&in_read, b"/tmp/inactu-fs/itest.txt");
...
fn hostcalls_queue_roundtrip_receipts() {
  ...
  (func (export "run") (result i32)
    ...
    run_bundle(
        ...
        &in_put,
        &root.join("receipt-put.json"),
        &[("INACTU_QUEUE_DIR", queue_dir.as_path())],
    );
