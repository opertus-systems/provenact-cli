...
use inactu_verifier::{parse_receipt_json, sha256_prefixed, verify_receipt_hash};
...
fn e2e_fixture_verify_run_verify_receipt() {
    ...
    let verify_receipt = Command::new(env!("CARGO_BIN_EXE_inactu-cli"))
...
fn pack_requires_allow_experimental_for_draft_schema_version() {
    ...
    let pack_without_gate = Command::new(env!("CARGO_BIN_EXE_inactu-cli"))
        .args(["pack", "--bundle"])
        .arg(&bundle_dir)
        .args(["--wasm"])
        .arg(&wasm_path)
    ...
    let pack_with_gate = Command::new(env!("CARGO_BIN_EXE_inactu-cli"))
        .args(["pack", "--bundle"])
        .arg(&bundle_dir)
        .args(["--wasm"])
        .arg(&wasm_path)
