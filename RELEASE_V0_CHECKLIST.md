...
- [x] Install semantics are implemented and validated:
  - `inactu install` supports local path / `file://` / `http(s)://` sources
...
- [x] Local content store contract is satisfied:
  - installed artifacts live under `~/.inactu/store/sha256/<hash>/`
  - local index metadata is maintained in `~/.inactu/index.json`
